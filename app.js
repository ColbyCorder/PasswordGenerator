...
// Function that returns a random number (AKA Random number a string value)
...
function generatePassword(lower, upper, number, symbol, length){
  ...
  // If the user has NOT selected any of the four options, then the  alert will be displayed and an empty string will be returned from the function so the password displays to user will be an empty string (AKA Nothing)
  if (typesCount === 0){
    alert(`Please select at least one option`);
    // The RETURN keyword stops/ends the execution of the function (AKA Does NOT run any of the lines of code that follow the return in the function)
    return '';
  }

  // Creating and array of arrays. The first item in each nested array holds the value of a string that will be used to access a function in the randomFunctions object. Also, the second items in each nested array are of the values passed into this generatePassword function
